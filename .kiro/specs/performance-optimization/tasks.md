...
- [x] 10. 最適化とチューニング
- [x] 10.1 パフォーマンスボトルネックの詳細分析

  - 各処理ステップの実行時間を詳細にログ出力
  - キャッシュヒット/ミスの統計を収集
  - スレッド待機時間の計測
  - _Requirements: 1.1, 1.2_

- [x] 10.2 キャッシュパラメータの最適化

  - 検出キャッシュ TTL を 0.5 秒 →1.0 秒に延長
  - フレーム類似度しきい値を 0.95→0.90 に緩和
  - OCR キャッシュの位置許容範囲を調整
  - _Requirements: 2.2, 4.1_

- [x] 10.3 検出処理の最適化

  - 信頼度しきい値を 0.6→0.7 に引き上げ（検出数削減）
  - 最大検出数の制限を強化
  - YOLOv8 の推論設定を最適化（imgsz, half precision 等）
